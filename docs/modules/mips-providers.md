...
- builtinAssemblerProvider.ts — phase-5 纯 TS 课程汇编器 provider：通过 source graph capture 读取 root/include blobs，核心经同一 Worker DTO 汇编出含 text/ktext/data/sourceMap 的 ProgramImage；写出课程 HexText/kernel dump，course-trace 模式验证标准停机自环；与 executor 同 id 注册在 legacy 之后，仅供显式 full-stack/shadow 解析，阶段 6 才切换默认
