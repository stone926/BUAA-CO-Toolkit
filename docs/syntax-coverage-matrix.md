...
本矩阵描述插件本地 MIPS 与 Verilog **LSP/parser** 当前覆盖的课程子集。它同时记录“已支持语法”和“课程外语法的处理方式”，用于避免后续 parser 改动把合法课程代码误报为语法错误。这里的“解析支持”不等于内置汇编器或 bundled Icarus 的执行支持；汇编/仿真边界以 README 的“内置工具与外部工具的边界”为准。
